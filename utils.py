...
# PlOTTING
################################################################################
def plot_aligned_manifolds(Z, X, Y, X_m, Y_m, attempt_rotation=False):
    """ Given original high-dimensional datasets X and Y, on manifolds
    X_m and Y_m, and low-dimensional embedding Z, plot everything."""
    N, _ = X.shape

    # try to put the first point in the top right quadrant to make it easier
    # to compare different alignments
    if attempt_rotation:
        index = np.argmax(Z[:, 0])
        if Z[index, 0] < 0:
            Z[:, 0] *= -1
    ...
    ax.scatter(xs=X[:,0], ys=X[:,1], zs=X[:,2], c=X_m, cmap=plt.cm.viridis)

    ax = fig.add_subplot(222)
    plt.scatter(X_embed[:,0], X_embed[:,1], c=X_m, cmap=plt.cm.viridis)

    ax = fig.add_subplot(223, projection='3d')
    ax.scatter(xs=Y[:,0], ys=Y[:,1], zs=Y[:,2], c=Y_m, cmap=plt.cm.viridis)

    ax = fig.add_subplot(224)
    plt.scatter(Y_embed[:,0], Y_embed[:,1], c=Y_m, cmap=plt.cm.viridis)

    plt.show()

def plot_aligned_MR(Z, resp):
    N = len(resp)
    assert Z.shape[0] == 2*N, 'Wrong shape of resp data'
    X_embed, Y_embed = Z[:N, :], Z[N:, :]
    fig = plt.figure(figsize=(16,8))
    ax = fig.add_subplot(121)
    plt.scatter(X_embed[:,0], X_embed[:,1], c=resp, cmap=plt.cm.viridis)
    ax = fig.add_subplot(122)
    plt.scatter(Y_embed[:,0], Y_embed[:,1], c=resp, cmap=plt.cm.viridis)
    plt.show()
